...
<body style="margin:0;padding:0;font-family:'Helvetica Neue',Arial,sans-serif;
  ...
  async function sendReport({ to, subject, message, summary, csvRows, headers, smtpConfig,
                              includeCSV = false, selectedCharts = {}, useMock = false }) {
    // Gather chart images
    ...
    if (!response.ok) {
      // If Java service is down (503), throw a special error
      if (response.status === 503) {
        const e = new Error(data.message || 'Java mail service unavailable');
        e.javaDown = true;
        e.hint = data.hint || '';
        throw e;
      }
