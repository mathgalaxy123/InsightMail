...
const LS = {
  ...
  getHistory() { try { return JSON.parse(localStorage.getItem(this.KEY_HISTORY) || '[]'); } catch{ return []; } },
  saveHistory(arr) { localStorage.setItem(this.KEY_HISTORY, JSON.stringify(arr)); },
...
function updateSummaryCards(s) {
  ...
  if (s.profitChange !== 'N/A') {
    ...
    profitEl.textContent = (up ? '▲ ' : '▼ ') + Math.abs(s.profitChange) + '% vs previous';
    profitEl.className = 'card-change ' + (up ? 'up' : 'down');
  } else {
    profitEl.textContent = 'No previous period data';
  ...
  document.getElementById('cardMarginChange').textContent =
    s.netProfit >= 0 ? '▲ Profitable period' : '▼ Loss period';
...
function renderRecentFiles() {
  const history = LS.getHistory().slice(0, 6);
    ...
    return `<tr>
      ...
      <td class="text-right">
        <div class="action-btns">
          ...
          <button class="btn-danger" onclick="historyDelete(${e.id})">Delete</button>
        </div>
      </td>
    </tr>`;
  }).join('');
...
window.historyView = function(id) {
  showToast('Reload the CSV file to view its report in Dashboard', 'info');
};

window.historyDelete = function(id) {
  let history = LS.getHistory().filter(e => e.id !== id);
